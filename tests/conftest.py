...
def test_user2(client):
    user_data = {"email": "kaban123@gmail.com",
...
def test_user(client):
    user_data = {"email": "kaban@gmail.com",
     "password": "password123"}
    res = client.post("/users/", json=user_data)
...
def test_posts(test_user, session, test_user2):
    ...
    def create_post_model(post):
        return models.Post(**post)
            # we taking dictionary and spreading it
    post_map = map(create_post_model, posts_data)
        # it returning map, in or
    posts = list(post_map)
        # now we converted from map to list
    ...
    # models.Post(title="3nd title", content="third content", owner_id = test_user["id"])]
