...
def test_get_all_posts(authorized_client, test_posts):
    ...
    # checking if we have 3 posts, basic test
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("pizza", "italy", True),
    ("sushi", "awesome new content", True),
    ("awesome new title", "akovbase", True)
])
...
def test_unautorized_user_create_post(client, test_posts):
...
def test_delete_test_success(authorized_client, test_user, test_posts):
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(
        "/posts/{test_posts/44444}", json=data)
